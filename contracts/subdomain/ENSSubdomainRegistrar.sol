/* Credit: AragonOS project: https://github.com/aragon/aragonOS licensed under GPL 3.0 */
...
contract ENSSubdomainRegistrar is  ENSConstants {
    ...
    */
    ...
    function setRootNode(bytes32 _rootNode) public {
        ...
        require(ens.owner(_rootNode) == address(this), ERROR_NO_NODE_OWNERSHIP);
        rootNode = _rootNode;
}
    ...
    */
    function createName(bytes32 _label, address _owner) external returns (bytes32 node) {
    ...
    * @return node Hash of created node
    */
    function createNameAndPoint(bytes32 _label, address _target) external returns (bytes32 node) {
        node = _createName(_label, address(this));
